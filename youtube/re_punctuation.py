...
#
# this is a fork of 
# https://github.com/oliverguhr/deepmultilingualpunctuation/tree/main/deepmultilingualpunctuation
# by Oliver Guhl
#
# with some modifications 
# 1.) to use mps instead of cuda
####################################
...
class PunctuationModel():
    def __init__(self, model = "oliverguhr/fullstop-punctuation-multilang-large") -> None:        
        if torch.cuda.is_available():
            ...
            self.pipe = pipeline("ner",model, aggregation_strategy="none", device="mps")
        else:
            self.pipe = pipeline("ner",model, aggregation_strategy="none")        

    def __str__(self):
        return str(self.pipe.model)
